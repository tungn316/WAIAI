...
use_openai_selection = st.sidebar.toggle(
    "Use OpenAI for final question selection",
    value=False,
    help=(
        "When ON, a second LLM call ranks the candidates. "
        "When OFF, rule-based willingness selection is used."
    ),
)
    ...
    with st.spinner(
        ...
        try:
            # Stage 5: Generate raw LLM candidates
                ...
                # Greedy selection (replaces CP-SAT)
                optimized, greedy_stats = greedy_select(
                        candidates=enriched,
                        question_budget=2,
                        token_budget=800,
                        beliefs=st.session_state.beliefs,
                ...
                # Convert to QuestionCandidate for followup selector
                candidates = [
                        raw_to_candidate(q) for q in optimized
                        ]
                ...
                if (
                    ...
                    result = select_followup_questions(
                            review_signals, candidates
                            )
